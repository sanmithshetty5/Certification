...
with st.sidebar:
    st.markdown(
        '<div style="font-size:1.5rem;font-weight:600;color:#ffffff;">🔍 Search Employee</div>',
        unsafe_allow_html=True
    )
...
with st.container(border=True):
    ...
    with c3:
        certification = st.selectbox(
            "Certification Track",
            certifications,
            index=certifications.index(certification)
            if certification in certifications else 0
        )

# -----------------------------------------
# Duplicate Check (NEW FEATURE)
# -----------------------------------------
...
# -----------------------------------------
# Duplicate Warning Message (NEW FEATURE)
# -----------------------------------------
...
with st.container(border=True):
    ...
    with m1:
        ...
        enrol_month = c_sub1.selectbox("Enrolment Month", month_opts)
        enrol_year = c_sub2.selectbox("Enrolment Year", year_opts)
