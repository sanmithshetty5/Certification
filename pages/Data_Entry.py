...
# --------------------------------------------------
# GLOBAL CSS – CLEAN ENTERPRISE DESIGN
# --------------------------------------------------
st.markdown("""
<style>
.stApp {
...
for key in [
    ...
    "last_emp_id", "save_completed", "autofill_emp_name", "duplicate_exists"
]:
    if key not in st.session_state:
        st.session_state[key] = None
...
with title_col:
    ...
    st.caption("Track, update, and manage employee certifications")
...
with st.container():
    ...
    st.markdown('<div class="section-title">👤 Employee & Certification</div>', unsafe_allow_html=True)

    c1, c2, c3 = st.columns([1.3, 2.5, 2])
    emp_id = c1.text_input("Employee ID", value=emp_id)
    emp_name = c2.text_input("Employee Name", value=autofill_employee_name(emp_id))
...
with st.container():
    st.markdown('<div class="form-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">🗓 Schedule & Status</div>', unsafe_allow_html=True)
    ...
    with left:
        m1, m2 = st.columns(2)
        enrol_month = m1.selectbox("Enrollment Month", month_opts)
        enrol_year = m2.selectbox("Enrollment Year", year_opts)
        planned_date = st.date_input("Planned Certification Date")

    with right:
        completed = st.checkbox("Certification Completed")
        if completed:
            actual_date = st.date_input("Actual Completion Date")
            snowpro = st.selectbox("SnowPro Status", ("Completed","Failed"))
        else:
            actual_date = None
            snowpro = "Incomplete"
            st.caption("Complete certification to unlock fields")

        voucher_status = st.selectbox(
            "Voucher Status", ("Voucher Received","Voucher Applied","Own Payment")
        )
...
with st.container():
    st.markdown('<div class="form-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">🏅 Badges & Preparation</div>', unsafe_allow_html=True)
...
with st.container():
    st.markdown('<div class="form-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">🏢 Department Information</div>', unsafe_allow_html=True)

    r1, r2 = st.columns(2)
    account = r1.text_input("Account")
    account_spoc = r1.text_input("Account SPOC")
    vertical = r2.text_input("Vertical / SL")
    batch = r2.text_input("Batch")

    comment = st.text_area("Comments", height=90)
...
with st.container():
    ...
    if a1.button("💾 Save Certification", type="primary", use_container_width=True):
        payload = {
            "EMP ID": emp_id,
            "EMP Name": emp_name,
            "Enrolment Month": f"{enrol_month}-{enrol_year}",
    ...
    a2.button("Cancel", use_container_width=True)
    a3.button("Delete", use_container_width=True)
