...
st.set_page_config(page_title="Realtime Analysis", layout="wide")

# ------------------------------------------------
# LOAD DATA (example – keep your existing loader)
# ------------------------------------------------
# df = your existing Snowflake query result

# SAFETY: ensure column exists
df.columns = df.columns.str.strip()

# Convert column to string & drop nulls safely
df["Enrolment Month"] = df["Enrolment Month"].astype(str)
df = df[df["Enrolment Month"] != "nan"]

# ------------------------------------------------
# SPLIT MONTH & YEAR (USER FRIENDLY)
# ------------------------------------------------
...
st.metric(
    label=f"Employees Enrolled in {selected_month}-{selected_year}",
    value=len(filtered_df)
)

# ------------------------------------------------
# GRAPH 1: Monthly Enrollment (existing logic safe)
# ------------------------------------------------
st.subheader("Enrollments for Selected Month & Year")

monthly_counts = (
    df.groupby(["Enroll_Month_Name", "Enroll_Year"])
      .size()
...
# ------------------------------------------------
# GRAPH 2: YEARLY COMPARISON (NEW)
# ------------------------------------------------
st.subheader("Year-wise Enrollment Comparison")
...
ax2.plot(
    yearly_counts["Enroll_Year"],
    yearly_counts["Employees"],
    marker="o"
)

ax2.set_xlabel("Year")
ax2.set_ylabel("Number of Employees")
ax2.set_title("Employees Enrolled Over the Years")
